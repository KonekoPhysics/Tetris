...
class block1():
    ...
    def roundRight(self):
        for i in range(1, 4):
            self.co[i] = [self.co[0][0] + (-1) * (self.co[i][1] - self.co[0][1]), self.co[0][1] + (self.co[i][0] - self.co[0][0])]
    def roundLeft(self):
        for i in range(1, 4):
            self.co[i] = [self.co[0][0] + (self.co[i][1] - self.co[0][1]), self.co[0][1] + (-1) * (self.co[i][0] - self.co[0][0])]
...
class view():
    ...
    def __init__(self):
        ...
        for y in range(0, 20):
            for x in range(0, 10):
                self.canvas.create_rectangle(30*x, 30*y, 30*x+30, 30*y+30, fill=self.CV_COLOR, outline="white", tag="x"+str(x)+"y"+str(y))
        self.minus=[[self.CV_COLOR for i in range(0, 10)] for j in range(0, 10)]
        self.blockLot = []
        self.canvas.pack()
        #キー操作の表示
        self.howtotext = " RIGHT: → or d Key \nLEFT: ← or a Key\nROUNDRIGHT: ↑ or s Key\nROUNDLEFT: e Key\nDOWN: ↓ or s Key\n\n EXIT: shift+c key"
        self.howto = tk.Label(self.root, text = self.howtotext, font=tkFont.Font(family="Consolas", size=11)).place(x = 310, y = 300)
        #得点表示板
        self.scoreNumber=0
        self.text = tk.StringVar()
        self.text.set("SCORE:" + str(self.scoreNumber))
        self.score=tk.Label(self.root, textvariable=self.text, font=tkFont.Font(family="Consolas", size=25), bg="white")
    ...
    def looping(self):
        ...
        self.state = 0
        #lambdaを使ったのはきまぐれと練習です許してくださいなんでもしますから！！！！！
        ...
        #ブロックを生成するか？
            ...
            for i in self.block.co:
                if any(self.canvas.itemcget("x"+str(i)+"y0", "fill") != self.CV_COLOR for i in range(0, 10)):
                    w = tk.Label(self.root, image = self.GAMEOVER).place(x = 60, y = 190)
                    x = tk.Label(self.root, text = "YOUR SCORE:" + str(self.scoreNumber), font=tkFont.Font(family="Consolas", size=20)).place(x = 130, y = 350)
                    x = tk.Label(self.root, text = "RESTART: push y key \n EXIT: push shift+c key", font=tkFont.Font(family="Consolas", size=15)).place(x = 120, y = 390)
                    self.root.bind("<Key-y>", lambda event: os.execl(sys.executable, sys.executable, *sys.argv))
                    return 0
            for i in self.block.co:
                if i[1] < 0:
    ...
    def deleting(self, a = 1):
        ...
        newcanvas = [[self.CV_COLOR for i in range(0, 10)] for j in range(0, 30)]
        ...
        for i in range(10, 30):
            for j in range(0, 10):
                newcanvas[i][j] = self.canvas.itemcget("x" + str(j) + "y" + str(i - 10), "fill")
        deletingnow = True
        while deletingnow:
            ...
            for i in range(10, 30):
                for j in range(0, 10):
                    if i - 10 >= 0:
                        self.canvas.itemconfigure("x" + str(j) + "y" + str(i - 10), fill = newcanvas[i][j])
    ...
    def judge(self, direction):
        ...
        if direction == "right":
            ...
            rightable = all((i[1] + 11 > 0 and i[1] < 29 and self.canvas.itemcget("x" + str(i[0] + 1) + "y" + str(i[1]), "fill") == self.CV_COLOR) for i in rights)
            return rightable
        ...
        if direction == "roundRight":
            judge = [[self.block.co[i][0], self.block.co[i][1]] for i in range(0, 4)]
            for i in range(1, 4):
                judge[i] = [judge[0][0] + (-1) * (judge[i][1] - judge[0][1]), judge[0][1] + (judge[i][0] - judge[0][0])]
            roundRightable = True
            for i in range(1, 4):
        ...
        if direction == "roundLeft":
            judge = [[self.block.co[i][0], self.block.co[i][1]] for i in range(0, 4)]
            for i in range(1, 4):
                judge[i] = [judge[0][0] + (judge[i][1] - judge[0][1]), judge[0][1] + (-1) * (judge[i][0] - judge[0][0])]
            roundLeftable = True
            for i in range(1, 4):
                if judge[i][0] >= 0 and judge[i][1] >= 0 and judge[i] not in self.block.co:
                    if self.canvas.itemcget("x"+str(judge[i][0])+"y"+str(judge[i][1]), "fill") != self.CV_COLOR:
        ...
        if direction == "down":
            ...
            downAble = all((i[1] + 11 > 0 and i[1] < 29 and self.canvas.itemcget("x" + str(i[0]) + "y" + str(i[1] + 1), "fill") == self.CV_COLOR) for i in bottoms)
            return downAble
            ...
            for i in self.block.co:
                ...
                for j in bottoms:
                    if bottoms.index(j) != len(bottoms) - 1 and j[0] == bottoms[-1][0]:
                        MIN = min(j[1], bottoms[-1][1])
                        bottoms.remove([j[0], MIN])
            downAble = all((i[1] < 29 and self.canvas.itemcget("x" + str(i[0]) + "y" + str(i[1] + 1), "fill") == self.CV_COLOR) for i in bottoms)
            return downAble
    ...
    def right(self, event):
        ...
        if self.state == 0:
            ...
            if self.judge("right"):
                for i in self.block.co:
                    self.canvas.itemconfigure("x"+str(i[0])+"y"+str(i[1]), fill=self.CV_COLOR)
                self.block.right()
                for i in self.block.co:
                    self.canvas.itemconfigure("x"+str(i[0])+"y"+str(i[1]), fill=self.block.color)
            self.state = 0
            return 0
        else:
            return 0
